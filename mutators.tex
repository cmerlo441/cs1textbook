...
A \textbf{parameterized constructor}\index{Methods!Constructors!Parameterized constructor}, as the name implies, is a constructor that has one or more parameters.  While every class needs to have a default constructor, not every class needs a parameterized one.  You can also provide more than one parameterized constructor in a class, if you find that it's appropriate to do so.  Whether you provide any is your choice, but it can make it easier for programmers to instantiate your class the way they want to.
